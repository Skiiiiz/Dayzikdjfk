class ActionUnsetClanBase: ActionInteractBase
{
	void ActionUnsetClanBase()
	{
		m_CommandUID = DayZPlayerConstants.CMD_ACTIONMOD_OPENDOORFW;
		m_StanceMask = DayZPlayerConstants.STANCEMASK_CROUCH | DayZPlayerConstants.STANCEMASK_ERECT;
		m_Text = "#STR_SMP_00746";
	}
	...
	override bool ActionCondition(PlayerBase player, ActionTarget target, ItemBase item)
	{
		...
		if (GetGame().IsClient())
		{
			...
			if (SM_ClanClientData.MyRank < SM_ClanClientData.GetActionRank(SM_ClanAction.SET_BASE))
				return false;
			if (vector.Distance(flag.GetPosition(), SM_ClanClientData.BasePos) > 2.0)
				return false;
		}
	...
	override void OnEndServer(ActionData action_data)
	{
		...
		if (flag && action_data.m_Player)
			SM_ClanManager.Get().UnsetClanBaseFromFlag(action_data.m_Player, flag);
	}
}
